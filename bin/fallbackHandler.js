...
import { PACKAGE_MANAGERS, isPackageManagerInstalled, performPackageManagerHealthCheck } from "./packageManager.js";
...
export async function attemptFallback(failedManager, error, excludeManagers = []) {
  ...
  console.log(chalk.blue(`🔍 Checking ${fallbackOrder.length} fallback option(s)...`));
  
  for (const fallbackManager of fallbackOrder) {
    const logEntry = logFallbackAttempt(failedManager, fallbackManager, error.message);
    
    try {
      // Check if fallback manager is installed
      const isInstalled = await isPackageManagerInstalled(fallbackManager);
      
      if (!isInstalled) {
        console.log(chalk.gray(`   ${fallbackManager} is not installed, skipping...`));
        updateFallbackResult(logEntry, false, "Not installed");
        continue;
      }
      
      // Perform health check
      const healthCheck = await performPackageManagerHealthCheck(fallbackManager);
      
      if (!healthCheck.healthy) {
        console.log(chalk.gray(`   ${fallbackManager} failed health check: ${healthCheck.error}`));
        updateFallbackResult(logEntry, false, healthCheck.error);
        continue;
      }
      
      // Fallback manager is available and healthy
      console.log(chalk.green(`✅ Successfully fell back to ${fallbackManager}`));
      ...
      console.log(chalk.gray(`   ${fallbackManager} failed: ${fallbackError.message}`));
  ...
  throw new Error(`All package managers failed. Original error: ${error.message}`);
}
...
export function getFallbackStats() {
  const stats = {
    ...
    successfulFallbacks: fallbackLog.filter(entry => entry.success === true).length,
    failedFallbacks: fallbackLog.filter(entry => entry.success === false).length,
    pendingFallbacks: fallbackLog.filter(entry => entry.success === null).length,
    mostCommonFailures: {},
    mostCommonFallbacks: {},
  ...
  fallbackLog.forEach(entry => {
    if (entry.reason) {
      stats.mostCommonFailures[entry.reason] = (stats.mostCommonFailures[entry.reason] || 0) + 1;
    }
    
    if (entry.to) {
      stats.mostCommonFallbacks[entry.to] = (stats.mostCommonFallbacks[entry.to] || 0) + 1;
    }
  });
...
export function displayFallbackSummary() {
  ...
  console.log(chalk.gray(`   Total fallback attempts: ${stats.totalAttempts}`));
  ...
  if (stats.successfulFallbacks > 0) {
    console.log(chalk.green(`   ✅ Fallback system helped avoid ${stats.successfulFallbacks} failure(s)`));
  }
  ...
  if (fallbackEntries.length > 0) {
    ...
    console.log(chalk.gray(`   Most used fallback: ${mostUsed[0]} (${mostUsed[1]} times)`));
  }
}
...
export async function selectIntelligentFallback(failedManager, context = {}) {
  ...
  if (scoredFallbacks.length > 0) {
    ...
    console.log(chalk.blue(`🎯 Intelligent fallback selected: ${bestFallback} (score: ${scoredFallbacks[0].score})`));
    return bestFallback;
  }
...
export async function handleOptimizationFallback(manager, originalFlags, error) {
  console.log(chalk.yellow(`⚠️  Optimized installation failed for ${manager.name}`));
  ...
  const fallbackStrategies = [
    {
      ...
      filter: flag => !flag.includes('silent') && !flag.includes('quiet')
    },
    {
      name: 'Remove offline flags', 
      filter: flag => !flag.includes('offline') && !flag.includes('prefer-offline')
    },
    {
      name: 'Remove parallel flags',
      filter: flag => !flag.includes('parallel') && !flag.includes('concurrent')
    },
    {
  ...
  for (const strategy of fallbackStrategies) {
    ...
    console.log(chalk.blue(`🔄 Trying fallback strategy: ${strategy.name}`));
    console.log(chalk.gray(`   Reduced flags: ${reducedFlags.join(' ') || '(none)'}`));
    ...
    try {
      ...
      console.log(chalk.gray(`   Strategy failed: ${strategyError.message}`));
      continue;
    }
  }
  
  throw new Error(`All optimization fallback strategies failed for ${manager.name}`);
}
