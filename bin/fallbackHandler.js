...
export async function attemptFallback(
  ...
  for (const fallbackManager of fallbackOrder) {
    ...
    try {
      ...
      if (!isInstalled) {
        console.log(
          chalk.gray(`   ${fallbackManager} is not installed, skipping...`),
        );
      ...
      const healthCheck =
        await performPackageManagerHealthCheck(fallbackManager);
      ...
      console.log(
        chalk.gray(`   ${fallbackManager} failed: ${fallbackError.message}`),
      );
...
export function getFallbackStats() {
  const stats = {
    ...
    successfulFallbacks: fallbackLog.filter((entry) => entry.success === true)
      .length,
...
export async function handleOptimizationFallback(
  ...
  for (const strategy of fallbackStrategies) {
    ...
    console.log(
      chalk.gray(`   Reduced flags: ${reducedFlags.join(" ") || "(none)"}`),
    );
