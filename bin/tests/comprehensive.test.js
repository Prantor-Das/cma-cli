...
import { detectProjectConfiguration, getPackageName } from '../lib/projectDetector.js';
import { updateConcurrentlyScripts } from '../lib/scriptGenerator.js';
import { getGitHubUsername, validateGitHubRepository, constructGitHubUrl, initializeGit } from '../lib/gitHandler.js';
import { LOCK_FILE_MANAGERS, CONCURRENTLY_DEPENDENCIES, GITHUB_URL_PATTERN, REPO_NAME_PATTERN } from '../lib/constants.js';
...
describe('CMA CLI Comprehensive Tests', () => {
  ...
  describe('Project Name Validation', () => {
    ...
    it('should reject invalid project names', () => {
      const invalidCases = [
        ...
        { name: 'my app', expectedError: 'Project name can only contain letters, numbers, hyphens, and underscores' },
        { name: 'my@app', expectedError: 'Project name can only contain letters, numbers, hyphens, and underscores' },
        { name: 'a'.repeat(215), expectedError: 'Project name is too long (max 214 characters)' }
      ];
  ...
  describe('Package Name Resolution', () => {
    it('should return package name from package.json', async () => {
      ...
      await fs.writeJson(path.join(clientDir, 'package.json'), { name: 'my-app-client' });
    ...
    it('should handle invalid package.json gracefully', async () => {
      const clientDir = path.join(testDir, 'client');
      await fs.ensureDir(clientDir);
      await fs.writeFile(path.join(clientDir, 'package.json'), 'invalid json');
  ...
  describe('Script Generation for Package Managers', () => {
    beforeEach(async () => {
      ...
      await fs.writeJson(path.join(testDir, 'client', 'package.json'), { name: 'test-app-client' });
      await fs.writeJson(path.join(testDir, 'server', 'package.json'), { name: 'test-app-server' });
    });

    const packageManagers = [
      { name: 'npm', command: 'npm', expectedClient: 'npm run dev --workspace client' },
      { name: 'yarn', command: 'yarn', expectedClient: 'yarn workspace test-app-client dev' },
      { name: 'pnpm', command: 'pnpm', expectedClient: 'pnpm --filter test-app-client dev' },
      { name: 'bun', command: 'bun', expectedClient: 'bun --filter test-app-client dev' }
    ];

    packageManagers.forEach(({ name, command, expectedClient }) => {
      it(`should generate ${name} scripts correctly`, async () => {
        const packageJsonPath = path.join(testDir, 'package.json');
        const packageJson = { scripts: {}, devDependencies: { concurrently: '^9.1.2' } };
  ...
  describe('Git Integration', () => {
    ...
    it('should get GitHub username from git config', async () => {
      ...
      expect(execa).toHaveBeenCalledWith('git', ['config', '--global', 'github.user']);
    });
    ...
    it('should fallback to user.name if github.user not set', async () => {
      ...
      expect(execa).toHaveBeenCalledWith('git', ['config', '--global', 'user.name']);
    });

    it('should validate GitHub repository URLs', async () => {
      vi.mocked(execa).mockResolvedValueOnce({});
      
      const result = await validateGitHubRepository('https://github.com/user/repo.git');
      
      expect(result).toBe(true);
      expect(execa).toHaveBeenCalledWith('git', ['ls-remote', '--heads', 'https://github.com/user/repo.git'], {
        timeout: 10000,
        stdio: 'ignore'
      });
    });
    ...
    it('should construct GitHub URLs correctly', () => {
      const testCases = [
        { input: 'https://github.com/user/repo.git', username: 'testuser', expected: 'https://github.com/user/repo.git' },
        { input: 'my-repo', username: 'testuser', expected: 'https://github.com/testuser/my-repo.git' },
        { input: '  my-repo  ', username: 'testuser', expected: 'https://github.com/testuser/my-repo.git' }
      ];
    ...
    it('should throw error for invalid repository formats', () => {
      ...
      expect(() => constructGitHubUrl('invalid/repo/name', 'testuser')).toThrow();
    });
    ...
    it('should initialize git repository successfully', async () => {
      ...
      expect(execa).toHaveBeenCalledWith('git', ['init'], { cwd: mockProjectPath });
      expect(execa).toHaveBeenCalledWith('git', ['remote', 'add', 'origin', 'https://github.com/testuser/my-repo.git'], { 
        cwd: mockProjectPath 
      });
    });
  });
  ...
  describe('End-to-End Project Creation', () => {
    beforeEach(async () => {
      ...
      await fs.writeJson(path.join(templateDir, 'client', 'package.json'), {
        name: 'client',
        scripts: { dev: 'vite' }
      });
      
      await fs.writeJson(path.join(templateDir, 'server', 'package.json'), {
        name: 'server',
    ...
    testConfigs.forEach(({ name, config, expectedScriptPattern, shouldHavePnpmWorkspace, clientOnly, serverOnly }) => {
      it(`should create project with ${name}`, async () => {
        ...
        if (clientOnly) {
          expect(await fs.pathExists(path.join(projectPath, 'client'))).toBe(true);
          expect(await fs.pathExists(path.join(projectPath, 'server'))).toBe(false);
          expect(await fs.pathExists(path.join(projectPath, 'package.json'))).toBe(false);
        } else if (serverOnly) {
          expect(await fs.pathExists(path.join(projectPath, 'server'))).toBe(true);
          expect(await fs.pathExists(path.join(projectPath, 'client'))).toBe(false);
          expect(await fs.pathExists(path.join(projectPath, 'package.json'))).toBe(false);
        } else {
          expect(await fs.pathExists(path.join(projectPath, 'client'))).toBe(true);
          expect(await fs.pathExists(path.join(projectPath, 'server'))).toBe(true);
          ...
          if (expectedScriptPattern) {
            expect(rootPackageJson.scripts.client).toContain(expectedScriptPattern);
          }
        ...
        if (!clientOnly) {
          const serverPackageJson = await fs.readJson(path.join(projectPath, 'server', 'package.json'));
          expect(serverPackageJson.name).toBe(`${config.projectName}-server`);
        }

        if (!serverOnly) {
          const clientPackageJson = await fs.readJson(path.join(projectPath, 'client', 'package.json'));
          expect(clientPackageJson.name).toBe(`${config.projectName}-client`);
        }
      });
    });
    ...
    it('should handle git initialization', async () => {
      ...
      expect(execa).toHaveBeenCalledWith('git', ['--version']);
      expect(execa).toHaveBeenCalledWith('git', ['init'], { cwd: path.join(testDir, config.projectName) });
    });
  ...
  describe('Questions Configuration', () => {
    ...
    it('should have package manager question with choices', () => {
      const packageManagerQuestion = questions.find(q => q.name === 'packageManager');
