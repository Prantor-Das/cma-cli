...
function determineShellSetupNeeds(packageManager, wasNewlyInstalled, availableInShell) {
  if (packageManager.name === "bun") {
    return !availableInShell;
  }
  return wasNewlyInstalled && !availableInShell;
}

async function displayShellSetupInstructions(packageManager) {
  console.log(chalk.yellow.bold(`\n⚠️  ${packageManager.name} was installed but is not available in your current shell.`));
  
  const setupInstructions = getShellSetupInstructions(packageManager.name);
  if (setupInstructions.length > 0) {
    console.log(chalk.yellow(setupInstructions[0]));
    setupInstructions.slice(1).forEach(instruction => {
...
async function displayNextSteps(config, packageManager, needsShellSetup) {
  ...
  const devCmd = useFullPath ? 
    `${fullPath} run ${config.language === "TypeScript" ? "build && " + fullPath + " run dev" : "dev"}` : 
  ...
  if (needsShellSetup) {
    console.log(chalk.yellow(`\n💡 Tip: After setting up your shell, you can use '${packageManager.name}' directly instead of the full path.`));
  }
...
function displaySeparateCommands(config, installCmd, devCmd, startCmd) {
  ...
  if (initializeParts === INIT_PARTS.BOTH || initializeParts === INIT_PARTS.CLIENT) {
    console.log(chalk.cyan("\n  To run the frontend:"));
  ...
  if (initializeParts === INIT_PARTS.BOTH || initializeParts === INIT_PARTS.SERVER) {
    const header = initializeParts === INIT_PARTS.BOTH ? 
...
async function run() {
  try {
    ...
    const availableInShell = await isPackageManagerAvailableInShell(packageManager.name);
    const needsShellSetup = determineShellSetupNeeds(packageManager, wasNewlyInstalled, availableInShell);
