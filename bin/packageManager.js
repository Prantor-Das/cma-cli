    ...
    if (config.optimizations) {
        ...
        if (config.optimizations.contextFlags) {
            ...
            for (const context of requiredContexts) {
                ...
                if (
                    !Array.isArray(config.optimizations.contextFlags[context])
                ) {
                    errors.push(
                        `Context flags for ${context} must be an array`,
                    );
                }
            }
        }
...
export async function resolvePackageManager(choice) {
    ...
    if (choice.startsWith("install-")) {
        ...
        console.log(
            chalk.yellow(
                "⚠️  Falling back to fastest available package manager...",
            ),
        );
    ...
    if (!healthCheck.healthy) {
        console.log(
            chalk.yellow(
                `⚠️  ${choice} failed health check: ${healthCheck.error}`,
            ),
        );
        console.log(
            chalk.yellow("⚠️  Using fastest available package manager..."),
        );
        return await detectFastestPackageManager();
    }
...
export async function installDependencies(
    ...
    try {
        ...
        if (report.cacheHitRate !== null) {
            console.log(
                chalk.gray(
                    `   Cache hit rate: ${(report.cacheHitRate * 100).toFixed(
                        1,
                    )}%`,
                ),
            );
...
async function installInPath(
    ...
    try {
        ...
        if (metrics) {
            ...
            if (result.stdout) {
                const packageMatches =
                    result.stdout.match(/(\d+)\s+packages?/i);
                if (packageMatches) {
        ...
        console.log(
            chalk.yellow(
                `⚠️  Optimized install failed, trying basic install...`,
            ),
        );
        ...
        try {
            ...
            if (metrics) {
                ...
                if (basicResult.stdout) {
                    const packageMatches =
                        basicResult.stdout.match(/(\d+)\s+packages?/i);
                    if (packageMatches) {
                        metrics.packagesInstalled += parseInt(
                            packageMatches[1],
                        );
                    }
                }
            ...
            console.log(
                chalk.yellow(
                    `⚠️  Basic install failed, trying minimal install...`,
                ),
            );
    ...
    try {
        for (const { path: checkPath, name } of pathsToCheck) {
            ...
            if (await fs.pathExists(nodeModulesPath)) {
                try {
                    const nodeModulesContents = await fs.readdir(
                        nodeModulesPath,
                    );
