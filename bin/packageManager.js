...
function createProgressIndicator(message, options = {}) {
  ...
  return {
    ...
    stop: (finalMessage) => {
      ...
      if (finalMessage) {
        if (showTimer) {
          process.stdout.write(`\r${finalMessage} ${chalk.gray(`(${totalTime}s)`)}\n`);
        } else {
...
export const PACKAGE_MANAGERS = {
  ...
  npm: {
    ...
    installInstructions: {
      macos: "Built-in with Node.js",
      linux: "Built-in with Node.js",
      windows: "Built-in with Node.js"
    },
    optimizations: {
...
function validatePackageManagerConfig(managerName, config) {
  ...
  const requiredFields = [
    'name', 'command', 'installCmd', 'ciCmd', 'lockFile', 
    'installFlags', 'globalInstall', 'speed', 'priority', 
    'installInstructions', 'optimizations', 'verification'
  ];
  ...
  if (typeof config.priority !== 'number' || config.priority < 1 || !Number.isInteger(config.priority)) {
    errors.push('Priority must be a positive integer');
  }
  
  // Validate installInstructions has required OS keys
  if (config.installInstructions) {
    const requiredOS = ['macos', 'linux', 'windows'];
    for (const os of requiredOS) {
      if (!(os in config.installInstructions)) {
        errors.push(`Missing install instructions for ${os}`);
      }
    }
  }
  ...
  if (config.optimizations) {
    const requiredOptFields = ['parallel', 'cache', 'offline', 'silent', 'concurrent', 'networkOptimized', 'contextFlags'];
    ...
    if (config.optimizations.contextFlags) {
      const requiredContexts = ['ci', 'development', 'production', 'fast', 'offline'];
  ...
  if (config.verification) {
    ...
    if (typeof config.verification.timeout !== 'number' || config.verification.timeout <= 0) {
...
function validateAllPackageManagerConfigs() {
  ...
  if (hasErrors) {
    console.error(chalk.red('❌ Package manager configuration validation failed:'));
...
export function getOptimizationFlags(managerName, context = 'fast') {
  ...
  if (!manager || !manager.optimizations || !manager.optimizations.contextFlags) {
    return [];
  }
...
export function supportsOptimization(managerName, optimization) {
  const manager = PACKAGE_MANAGERS[managerName];
  return manager && manager.optimizations && manager.optimizations[optimization] === true;
}
...
try {
  ...
  console.warn(chalk.yellow('⚠️  Package manager configuration validation failed, some features may not work correctly'));
}
...
async function checkBunInCommonPaths() {
  ...
  if (os === "windows") {
    ...
    try {
      const npmBinResult = await execa("npm", ["bin", "-g"], { stdio: "pipe", timeout: 5000 });
  ...
  for (const bunPath of commonPaths) {
    try {
      if (await fs.pathExists(bunPath)) {
        // Try to execute it
        await execa(bunPath, ["--version"], { stdio: "ignore", timeout: 3000 });
...
// Enhanced installation result interface
export class InstallationResult {
  constructor(success, managerName, installedPath = null, needsShellSetup = false, error = null, retryCount = 0) {
    this.success = success;
    this.managerName = managerName;
    this.installedPath = installedPath;
    this.needsShellSetup = needsShellSetup;
    this.error = error;
...
// Enhanced installation verification with retry mechanisms
async function verifyInstallationWithRetry(managerName, maxRetries = 3, retryDelay = 2000) {
  const manager = PACKAGE_MANAGERS[managerName];
  if (!manager) return false;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Use enhanced verification with health check
      const isInstalled = await isPackageManagerInstalled(managerName);
      if (isInstalled) {
        // Perform additional health check
        const healthCheck = await performPackageManagerHealthCheck(managerName);
...
// Update environment PATH for newly installed managers
function updateEnvironmentPath(managerName, installedPath) {
  const manager = PACKAGE_MANAGERS[managerName];
  if (!manager || !installedPath) return false;
  
  const binDir = path.dirname(installedPath);
...
// Get installation method for a package manager
function getInstallationMethod(managerName, os) {
  const manager = PACKAGE_MANAGERS[managerName];
  if (!manager) return null;
  
  const instruction = manager.installInstructions[os];
  
  if (instruction === "Built-in with Node.js") {
    return { type: 'builtin', command: null };
  }
  ...
  if (managerName === "bun") {
    if (os === "windows") {
      return {
        ...
        env: { BUN_INSTALL_SILENT: "1" }
      };
    } else {
      return {
        type: 'curl_bash',
        command: ["bash", ["-c", "curl -fsSL https://bun.sh/install | bash"]],
        env: { BUN_INSTALL: path.join(process.env.HOME, ".bun") }
      };
    }
  }
  
  // Default npm installation for pnpm/yarn
  return {
    type: 'npm',
    command: ["npm", ["install", "-g", managerName, "--silent"]],
    env: {}
  };
}

// Enhanced package manager installation with comprehensive error handling
export async function installPackageManager(managerName, options = {}) {
  const {
    maxRetries = 2,
    retryDelay = 2000,
    timeout = 120000,
  ...
  const manager = PACKAGE_MANAGERS[managerName];
  if (!manager) {
    return new InstallationResult(false, managerName, null, false, `Unknown package manager: ${managerName}`);
  }
  
  const os = getOS();
  let spinner = createProgressIndicator(`Installing ${manager.name}...`);
  
  // Check if it's a built-in manager
  if (manager.installInstructions[os] === "Built-in with Node.js") {
    spinner.stop(chalk.yellow(`⚠️  ${managerName} should be built-in with Node.js`));
    return new InstallationResult(false, managerName, null, false, "npm should already be available with Node.js");
  }
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt > 1) {
        spinner = createProgressIndicator(`Retrying ${manager.name} installation (attempt ${attempt})...`);
      }
      
      const installMethod = getInstallationMethod(managerName, os);
      if (!installMethod) {
        throw new Error(`No installation method available for ${managerName} on ${os}`);
      }
      
      const [command, args] = installMethod.command;
      const env = { ...process.env, ...installMethod.env };
      ...
      const verification = await verifyInstallationWithRetry(managerName, 3, 1000);
      
      if (verification.success) {
        // Get installed path and update environment
        const installedPath = await getPackageManagerPath(managerName);
        const pathUpdated = installedPath ? updateEnvironmentPath(managerName, installedPath) : false;
        
        spinner.stop(chalk.green(`✅ ${manager.name} installed successfully!`));
        ...
        throw new Error(`Installation verification failed after ${verification.attempt} attempts: ${verification.error || 'Unknown verification error'}`);
      }
      ...
      if (attempt < maxRetries) {
        spinner.stop(chalk.yellow(`⚠️  Installation attempt ${attempt} failed, retrying...`));
        await new Promise(resolve => setTimeout(resolve, retryDelay));
      }
    }
  }
  ...
  if (fallbackMethods && managerName === "bun") {
    try {
      spinner.stop(chalk.yellow(`⚠️  Primary installation failed, trying npm fallback...`));
      const fallbackSpinner = createProgressIndicator(`Installing ${manager.name} via npm...`);
      totalRetries++;
      
      // Try installing via npm as fallback
      await execa("npm", ["install", "-g", "bun", "--silent"], {
        stdio: "pipe",
        timeout: timeout
      });
      
      // Update PATH with npm global bin
      try {
        const npmBinResult = await execa("npm", ["bin", "-g"], { stdio: "pipe" });
        const globalBinPath = npmBinResult.stdout.trim();
        updateEnvironmentPath(managerName, path.join(globalBinPath, "bun"));
      } catch (pathError) {
        // PATH update failed, but installation might still work
      }
      ...
      if (verification.success) {
        ...
        fallbackSpinner.stop(chalk.green(`✅ ${manager.name} installed via npm fallback!`));
        
        return new InstallationResult(
          true, 
          managerName, 
          installedPath, 
          true, // npm fallback usually needs shell setup
          null, 
          totalRetries
        );
      } else {
        fallbackSpinner.stop(chalk.red(`❌ Fallback installation verification failed`));
      }
      
    } catch (fallbackError) {
      lastError = fallbackError;
      console.log(chalk.red(`❌ Fallback installation failed: ${fallbackError.message}`));
    }
  }
  
  // All installation attempts failed
  spinner.stop(chalk.red(`❌ Failed to install ${manager.name} after ${totalRetries} attempts`));
  console.log(chalk.yellow(`💡 Manual installation: ${manager.installInstructions[os]}`));
  ...
  return new InstallationResult(
    ...
    null, 
    false, 
    lastError?.message || 'Installation failed after all retry attempts', 
    totalRetries
  );
}
...
export async function resolvePackageManager(choice) {
  ...
  if (choice.startsWith("install-")) {
    ...
    console.log(chalk.blue(`🔧 Setting up ${managerName}...`));
    
    const installResult = await installPackageManager(managerName);
    
    if (!installResult.success) {
      console.log(chalk.yellow(`⚠️  Installation failed: ${installResult.error}`));
      console.log(chalk.yellow("⚠️  Falling back to fastest available package manager..."));
      const fallback = await detectFastestPackageManager();
      console.log(chalk.blue(`📦 Using ${fallback.name} ${fallback.speed} instead`));
      return fallback;
    }
    
    // Show shell setup instructions if needed
    if (installResult.needsShellSetup) {
      const setupInstructions = getShellSetupInstructions(managerName);
    ...
    if (installResult.retryCount > 1) {
      console.log(chalk.green(`✅ ${managerName} installed after ${installResult.retryCount} attempts!`));
    } else {
      console.log(chalk.green(`✅ ${managerName} is ready to use!`));
    }
    
    return PACKAGE_MANAGERS[managerName];
  }
  
  // Handle direct choice
  const manager = PACKAGE_MANAGERS[choice];
  if (!manager) {
    console.log(chalk.yellow(`⚠️  Unknown package manager: ${choice}. Using fastest available...`));
    return await detectFastestPackageManager();
  }
  
  // Verify the manager is still installed with enhanced verification
  const isInstalled = await isPackageManagerInstalled(choice);
  if (!isInstalled) {
    console.log(chalk.yellow(`⚠️  ${choice} is no longer available. Using fastest available...`));
    return await detectFastestPackageManager();
  }
  ...
  if (!healthCheck.healthy) {
    console.log(chalk.yellow(`⚠️  ${choice} failed health check: ${healthCheck.error}`));
...
export async function installDependencies(projectPath, packageManager, concurrently, initializeParts = "both") {
  console.log(chalk.blue(`📦 Using ${packageManager.name} ${packageManager.speed} for installation...`));
  ...
  try {
    ...
    if (concurrently) {
      ...
      if (initializeParts === "both" || initializeParts === "client") {
        installPaths.push({ path: path.join(projectPath, "client"), name: 'client' });
      }
      if (initializeParts === "both" || initializeParts === "server") {
        installPaths.push({ path: path.join(projectPath, "server"), name: 'server' });
      }
    }
    ...
    for (let i = 0; i < installPaths.length; i++) {
      ...
      if (concurrently || installPaths.length === 1) {
        ...
        installPromises.push(installInPath(installPath, packageManager, 'fast', metrics));
      }
    }
    ...
    progressIndicator.stop(chalk.green(`✅ Dependencies installed successfully`));
    
    metrics.complete(true);
    
    // Show installation summary
    const report = metrics.getReport();
    console.log(chalk.green(`📊 Installation completed in ${report.totalDuration.toFixed(1)}s using ${packageManager.name}`));
    
    // Show detailed breakdown if multiple phases
    if (report.phases.length > 1) {
      console.log(chalk.gray(`   Breakdown:`));
      report.phases.forEach(phase => {
        console.log(chalk.gray(`   • ${phase.name}: ${phase.duration.toFixed(1)}s`));
      });
    }
    ...
    if (report.cacheHitRate !== null) {
      console.log(chalk.gray(`   Cache hit rate: ${(report.cacheHitRate * 100).toFixed(1)}%`));
    }
    ...
    const report = metrics.getReport();
    console.error(chalk.red(`❌ Dependency installation failed after ${report.totalDuration.toFixed(1)}s: ${error.message}`));
    console.log(chalk.yellow(`💡 You can install manually later with: ${getInstallCommand(packageManager)}`));
...
async function installInPath(installPath, packageManager, context = 'fast', metrics = null) {
  const hasLockFile = await fs.pathExists(path.join(installPath, packageManager.lockFile));
  ...
  if (hasLockFile && context !== 'ci') {
    ...
    commandArgs = [packageManager.installCmd, ...packageManager.installFlags, ...optimizationFlags];
  }
  ...
  try {
    ...
    console.log(chalk.yellow(`⚠️  Optimized install failed, trying basic install...`));
    const basicArgs = hasLockFile ? packageManager.ciCmd.split(" ") : [packageManager.installCmd];
    ...
    try {
      ...
      console.log(chalk.yellow(`⚠️  Basic install failed, trying minimal install...`));
      
      if (metrics) {
        metrics.retryCount++;
      }
      
      const minimalResult = await execa(packageManager.command, [packageManager.installCmd], {
        cwd: installPath,
        stdio: "pipe",
        timeout: 300000,
      });
      ...
      if (metrics) {
        ...
        if (minimalResult.stdout) {
          const packageMatches = minimalResult.stdout.match(/(\d+)\s+packages?/i);
          if (packageMatches) {
            metrics.packagesInstalled += parseInt(packageMatches[1]);
          }
        }
...
// Get shell setup instructions for newly installed package managers
export function getShellSetupInstructions(managerName) {
  const os = getOS();
  
  if (managerName === "bun") {
    if (os === "windows") {
      return [
        "To use bun in new terminals, restart your terminal or run:",
        "refreshenv"
      ];
    } else {
      ...
      return [
        "To use bun in new terminals, run one of these commands:",
        `source ${rcFile}`,
...
export async function performPostInstallationVerification(projectPath, packageManager, initializeParts = "both") {
  ...
  if (initializeParts === "both" || initializeParts === "client") {
    pathsToCheck.push({ path: path.join(projectPath, "client"), name: "client" });
  }
  if (initializeParts === "both" || initializeParts === "server") {
    pathsToCheck.push({ path: path.join(projectPath, "server"), name: "server" });
  }
  if (pathsToCheck.length === 0) {
    pathsToCheck.push({ path: projectPath, name: "project" });
  }
  
  const spinner = createProgressIndicator("Verifying installations", { showTimer: true });
  ...
  try {
    for (const { path: checkPath, name } of pathsToCheck) {
      ...
      if (await fs.pathExists(lockFilePath)) {
        verificationResults.lockFiles.push(`${name}/${packageManager.lockFile}`);
      } else {
        verificationResults.warnings.push(`No lock file found in ${name} (${packageManager.lockFile})`);
      }
      ...
      if (await fs.pathExists(nodeModulesPath)) {
        try {
          ...
          const packageCount = nodeModulesContents.filter(item => !item.startsWith('.')).length;
          verificationResults.nodeModules.push(`${name}: ${packageCount} packages`);
          verificationResults.summary[name] = { packages: packageCount };
        } catch (error) {
          verificationResults.warnings.push(`Could not read node_modules in ${name}: ${error.message}`);
        }
      } else {
        verificationResults.errors.push(`Missing node_modules directory in ${name}`);
        verificationResults.success = false;
      }
      ...
      try {
        ...
        verificationResults.warnings.push(`Package manager verification failed in ${name}: ${error.message}`);
      }
    }
    ...
    verificationResults.success = false;
    verificationResults.errors.push(`Verification process failed: ${error.message}`);
    return verificationResults;
  }
}

// Generate installation report with recommendations
export function generateInstallationReport(metrics, verificationResults, packageManager) {
  const report = {
  ...
  if (report.performance.totalDuration > 60) {
    report.recommendations.push("Consider using a faster package manager like bun or pnpm for better performance");
  }
  
  if (report.performance.cacheHitRate !== null && report.performance.cacheHitRate < 0.5) {
    report.recommendations.push("Low cache hit rate detected. Consider clearing and rebuilding package manager cache");
  }
  
  if (report.performance.retryCount > 0) {
    report.recommendations.push("Installation required retries. Check network connection and registry availability");
  }
  
  // Add verification recommendations
  if (!report.verification.success) {
    report.recommendations.push("Installation verification failed. Manual verification recommended");
  }
  
  if (report.verification.warnings.length > 0) {
    report.recommendations.push("Installation completed with warnings. Review warning messages above");
  }
  
  if (report.verification.lockFiles.length === 0) {
    report.recommendations.push("No lock files were created. Consider running the install command again");
  }
...
export async function performPackageManagerHealthCheck(managerName) {
  const manager = PACKAGE_MANAGERS[managerName];
  if (!manager || !manager.verification) return { healthy: false, error: 'No verification config' };
  ...
  try {
    ...
    return { 
      ...
      command: `${manager.command} ${manager.verification.healthCheck.join(' ')}`
    };
  }
}
