...
const rootDirectory = [{ path: ".", name: "Root" }];
...
function showHelp() {
  ...
  console.log(
    "  --all                         Install in all directories (default)",
  );
  console.log(
    "  --pm, --package-manager <pm>  Force specific package manager (npm, yarn, pnpm, bun)",
  );
...
async function installInDirectory(dirPath, dirName, forcedPM = null) {
  if (!hasPackageJson(dirPath)) {
    console.log(
      colors.yellow(`⚠️  Skipping ${dirName}: No package.json found`),
    );
  ...
  console.log(
    colors.cyan(
      `📦 Installing packages in ${dirName} using ${packageManager}...`,
    ),
  );
  ...
  try {
    ...
    console.log(
      colors.green(`✓ ${dirName} installation completed in ${duration}s`),
    );
...
async function installPackages(options) {
  ...
  const scopeText =
    options.scope === "root" ? "root directory" : "all directories";
  const pmText = options.packageManager
    ? ` using ${options.packageManager}`
    : "";

  console.log(
    colors.blue(
      `📦 Starting package installation for ${scopeText}${pmText}...\n`,
    ),
  );
  ...
  for (const dir of directories) {
    const result = await installInDirectory(
      dir.path,
      dir.name,
  ...
  if (failed === 0) {
    console.log(
      colors.green("🎉 All package installations completed successfully!"),
    );
  } else {
    console.log(
      colors.yellow("⚠️  Some installations failed. Check the errors above."),
    );
    process.exit(1);
  }
}
...
if (
  ...
  console.error(
    colors.red(`❌ Invalid package manager: ${options.packageManager}`),
  );
